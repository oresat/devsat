...
//Data defined for DIO output
static uint8_t toggle_bit = 0;
static uint8_t state_bit = 0;
static uint8_t count_bit = 0;
static uint8_t output_bit = 0;
...
static void gpt3cb(GPTDriver *gptp) {
	  ...
	  (void)gptp;
	  //toggles to create the DCLK signal on D01
	  /*if (toggle_bit==0){
		toggle_bit = 1;
	  ...
	  palWritePad(GPIOC, GPIOC_SX_DIO1,toggle_bit);
	  
	  if (state_bit == 0){				// simple state machine. state_bit=0 means send preamble
		if (toggle_bit == 0){
			if (output_bit == 0){
				output_bit = 1;
			}else{
	  ...
	  if (state_bit == 1){				// simple state machine. state_bit=1 means send 1s	
		output_bit = 1;
		if (count_bit == 254){
			count_bit = 0;
			state_bit = 2;
	  ...
	  if (state_bit == 2){				// simple state machine. state_bit=2 means send 0s	
		output_bit = 0;
		if (count_bit == 254){
			count_bit = 0;
			state_bit = 1;
	  ...
	  if ( toggle_bit==0){
		  palWritePad(GPIOC, GPIOC_SX_DIO2,output_bit);
		  palWritePad(GPIOA, GPIOA_SX_TESTOUT,output_bit);
	  }*/
...
{
    (void)id;
    chprintf(DEBUG_CHP, "dio1 event\r\t");
    count_bit = count_bit + 1;
...
static THD_FUNCTION(Thread_sx1236_dio, arg)
{
    ...
    while (TRUE)
    {
        chEvtDispatch(evhndl_sx1236_dio, chEvtWaitOne(EVENT_MASK(1)));
